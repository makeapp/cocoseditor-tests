...
var TestController = cc.LayerGradient.extend({
    ...
    ctor: function ()
    {
        ...
        for (var i = 0, len = testNames.length; i < len; i++) {
            ...
            if (i == 0) {
                var label = cc.LabelTTF.create(testNames[i].title, "Marker Felt", 40);
                var menuItem = cc.MenuItemLabel.create(label, this.onMenuCallback, this);
                this._itemMenu.addChild(menuItem, i + 10000);
                menuItem.setPosition(winSize.width / 2, (winSize.height - 20));
                menuItem.setColor(cc.c3b(255, 0, 0));
            }
            else {
